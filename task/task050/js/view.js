...
//wrap("can1",a,b,c);
$("#title>h3").text(data[0].title);
for (var i=0;i<data[0].questions.length;i++){
    if (data[0].questions[i].typle=="radius"){
        ...
        $(question).find("div.left header span:last-of-type").text(data[0].questions[i].text);
        var as=[];
        var totle=0;
        for (var j=0;j<data[0].questions[i].option.length;j++){
            as[j]=data[0].questions[i].option[j];
            $(question).find("div.left li").eq(j).text(data[0].questions[i].option[j]);
            totle+=data[0].questions[i].per[j];
        }
        for (var k=0;k<=j;k++){
            var width=Math.round((data[0].questions[i].per[k]/totle)*1000)/10;
        ...
        for (var k=0;k<data[0].questions[i].per.length;k++){
            perl[k]=data[0].questions[i].per[k]/totle;
        }
        wrap("q"+i,data[0].questions[i].option,perl,color);
        console.log(data[0].questions[i].option)
    }
